...
CUDA_VISIBLE_DEVICES=${cuda_device} nohup python run_disfluency_finetune_eval_each_electra_st.py --task_name disfluency --do_train --do_eval --do_test --do_lower_case --bert_model bert_model/electra_en_base/ --max_seq_length 128 --train_batch_size 64 --gradient_accumulation_steps 1 --learning_rate 2e-5 --num_train_epochs 1.0 --do_tagging --pretrain_model_dir self_supervised_model/ --pretrain_model_name pytorch_model.bin --model_name_or_path bert_model/electra_en_base/ --use_new_model --unlabel_size 0 --seed ${data_scale} --data_dir run_data/${data_scale}/ --output_dir run_model/${data_scale}/
wait
